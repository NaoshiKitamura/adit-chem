...
|---|---|---|---|
| `si.proj.projwfc_up` | 手元で Quantum ESPRESSO 7.x の projwfc.x を Si 2 原子で実走 | — (自分で作った) | 2026-09-13。filproj の並び |
| `epsr_si.dat` / `epsi_si.dat` / `eels_si.dat` | 手元で QE の epsilon.x を Si (ノルム保存の擬ポテンシャル) で実走 | — (自分で作った) | 2026-09-13。列の並びと、-Im(1/eps) が QE 自身の値と一致すること |
| `bader_ACF.dat` | 手元で bader 1.0.5 を、電子数 6 と 2 のガウス関数を置いた cube で実走 | — (自分で作った) | 2026-09-13。ACF.dat の列と、分けられた電子数 (5.9999 / 1.9987) |
...
| `water_hessian.out` | DFTB+ の実走 (2026-09-10) | — (自分で作った) | hessian.out の並び |
