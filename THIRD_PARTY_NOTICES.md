...
下の第三者のファイルは、GitHub のリポジトリにだけ入っています (2026-09-14 に実際の配布物の中身で確認)。

## 計算ソフトの配布物から取ったファイル

| ファイル | 出典 | ライセンス | 全文 |
|---|---|---|---|
| `examples/lammps_cu/Cu_u3.eam` | LAMMPS の `potentials/` | GPL-2.0 | `licenses/GPL-2.0.txt` |
...
| `examples/gromacs_spce/conf.gro` ほか GROMACS の例 | GROMACS の `share/gromacs/top/spc216.gro` | LGPL-2.1 | `licenses/LGPL-2.1.txt` |
...
- **Slater-Koster パラメータ (slakos/)**: 利用者が dftb.org から入れます。リポジトリには入れていません (`.gitignore`)
...
**その全文 `licenses/OFL-1.1-NotoSansCJK.txt` も一緒に入ります** (2026-09-15 に `packaging/adit.spec` へ仕込みました。
...
**PySide6 (Qt) は LGPL-3.0 です。**画面つきの実行ファイルを配るときは、LGPL の求めに応じて
