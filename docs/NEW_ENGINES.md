...
このページの 7 コードは **CLI での限定的な入力生成**に対応します。ADIT は `spec.json` を読み、`adit-gen spec.json out/` で入力ファイル・`submit.sh`・生成したファイルの説明を作ります。計算は実行・投入しません。この 7 コードの結果解析もまだ行いません。GUIとウェブ版には操作欄を追加していません。
