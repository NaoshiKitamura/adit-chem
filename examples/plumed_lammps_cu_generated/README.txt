...
== 研究室のクラスタで走らせたいとき ==
  ...
  2. クラスタにログインして、写したディレクトリへ移動します (ssh = 別の計算機にログインするコマンド)
       ssh <ユーザー名>@<クラスタのホスト名>
       cd <クラスタでの作業ディレクトリ>/regen_plumed
  ...
  5. 終わったら結果を手元に取り戻します (手元の PC のターミナルで)
       scp -r <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/regen_plumed <手元の置き場所>/
     または
       rsync -av <ユーザー名>@<クラスタのホスト名>:<クラスタでの作業ディレクトリ>/regen_plumed <手元の置き場所>/
...
== 引用 ==
  ...
  PLUMED も引用してください (使った機能の文献は PLUMED の出力の最後に並びます)。

== 来歴 ==
  ADIT 0.0.1 / Python 3.14.7 / ASE 3.29.0 / 生成 2026-09-12T21:21:03+00:00 (UTC)
